...
const APIKey = "a795539b71a04c8fbe6163616250307";
...
getUserCity(); // Call  function to get user city
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          ...
          data.city
            ? (searchLocation = normalizeCityName(data.city))
            : (searchLocation = "Alexandria");
          displayWeather();
        } catch (err) {
...
async function getWeather(searchLocation) {
  try {
    let response = await fetch(
      `${baseURL}?key=${APIKey}&q=${searchLocation}&days=3`
    );
      ...
      <div class="col-lg-4">
          <div class=" forecast today rounded  rounded-3">
              <div id="today" class="forecast-header px-3  d-flex justify-content-between">
                  <div class="day fw-semibold">${handleDay(
                    locationObj.localtime
                  )}</div>
                  <div class="date text-light">${new Date(
                    locationObj.localtime.split(" ")[0]
                  ).getDay()} ${handleMonth(
    locationObj.localtime.split(" ")[0]
  )}</div>
                  </div>
                  <div id="current" class="forecast-content  px-3">
                  <div class="location fs-4">${locationObj.country},${
    locationObj.name
  }</div>
                  <div class="time me-auto">update: ${
                    locationObj.localtime.split(" ")[1]
                  }</div>
          ...
          <div class="text-center h-100 rounded-2 forecast tomorrow">
              ...
              <div class="forecast-content px-3">
                  <div class="forecast-icon">
                      <img src=${
                        forecastArr[1].day.condition.icon
                      } alt="weather icon"
                          width="90">
                  </div>
                  <div class="degree fs-2">${
                    forecastArr[1].day.maxtemp_c
                  }<sup>o</sup>C</div>
                  <p class="fs-5 mb-0">${
                    forecastArr[1].day.mintemp_c
                  }<sup>o</sup></p>
                  <p class="custom fs-5">${
                    forecastArr[1].day.condition.text
                  }</p>
              </div>
          </div>
      </div>
      ...
      <div id="afterTomorrow" class="col-lg-4 mt-3 mt-lg-0">
          <div class="text-center h-100 rounded-2 forecast">
              ...
              <div class="forecast-content px-3">
                  ...
                  <div class="degree fs-2">${
                    forecastArr[2].day.maxtemp_c
                  }<sup>o</sup>C</div>
                  <p class="fs-5 mb-0">${
                    forecastArr[2].day.mintemp_c
                  }<sup>o</sup></p>
                  <p class="custom fs-5">${
                    forecastArr[2].day.condition.text
                  }</p>
              </div>
          </div>
      </div>
